...
from telethon.tl.types import (
    ...
    MessageService, MessageActionStarGift,
    InputSavedStarGiftUser, PeerUser,
)
...
# events.NewMessage не гарантированно ловит СЕРВИСНЫЕ сообщения (а подарок —
# это MessageService, не обычное сообщение). events.Raw слушает вообще все
# апдейты напрямую, без внутренней фильтрации Telethon — надёжнее для этого случая.
@bank_client.on(events.Raw(types=[UpdateNewMessage, UpdateNewChannelMessage]))
async def handle_incoming_gift(update):
    ...
    logger.info(f"[gift] Получено сервисное сообщение с подарком, msg_id={message.id}")

    if message.out:
        logger.info("[gift] Это исходящее сообщение (наше собственное) — игнорирую")
        return

    # В приватном чате peer_id указывает на собеседника (для входящих сообщений это отправитель)
    sender_id = None
    ...
    if sender_id is None:
        logger.warning(f"[gift] Не удалось определить отправителя, message={message.stringify()}")
        return

    original_amount = action.gift.stars
    resold_amount = action.convert_stars
    logger.info(
        f"[gift] От {sender_id}: номинал={original_amount}, "
        f"можно конвертировать в={resold_amount}"
    )

    if not resold_amount:
        logger.warning(f"[gift] Подарок от {sender_id} нельзя конвертировать (convert_stars=0)")
        try:
            await bot.send_message(
                chat_id=ADMIN_CHAT_ID,
                text=(
    ...
    try:
        result = await bank_client(ConvertStarGiftRequest(
            stargift=InputSavedStarGiftUser(msg_id=message.id)
        ))
        logger.info(f"[gift] Подарок продан, результат: {result}")
    except Exception as e:
        logger.error(f"[gift] Не удалось продать подарок от {sender_id}: {e}")
        try:
            await bot.send_message(
                chat_id=ADMIN_CHAT_ID,
    ...
    logger.info(f"[gift] Баланс пользователя {sender_id} обновлён")
